...
class Downloader:
    ...
    def __init__(self, code_dir, use_pydrive, subdir):
        ...
        self.save_dir = os.path.join(os.path.dirname(
            current_directory), code_dir, subdir)
    ...
    def download_file(self, file_id, file_name):
        ...
        command = self._get_download_model_command(
    ...
    def _get_download_model_command(self, file_id, file_name):
        ...
        url = r"""wget --load-cookies /tmp/cookies.txt "https://docs.google.com/uc?export=download&confirm=$(wget --quiet --save-cookies /tmp/cookies.txt --keep-session-cookies --no-check-certificate 'https://docs.google.com/uc?export=download&id={FILE_ID}' -O- | sed -rn 's/.*confirm=([0-9A-Za-z_]+).*/\1\n/p')&id={FILE_ID}" -O {SAVE_PATH}/{FILE_NAME} && rm -rf /tmp/cookies.txt""".format(
            FILE_ID=file_id, FILE_NAME=file_name, SAVE_PATH=self.save_dir)
        return url
...
def crop_image(image_path):
    ...
    cropped_image = crop_face(filepath=str(
        image_path), detector=detector, predictor=predictor)
